export {createElement, loadElementFrom, clearNode, saveToFile, saveToGithub, loadFromGithub, md5}

function createElement(tag, id, parent, attributes){
  let node = document.createElement(tag); node.id = id;
  if(typeof(attributes) === 'object'){for(const key in attributes){node.setAttribute(key,attributes[key])}}
